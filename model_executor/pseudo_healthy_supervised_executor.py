...
from callbacks.image_call_back_pseudo_healthy_supervised import ImageCallback_pseudo_healthy_sup as ImageCallback
from models.pseudo_healthy_supervised import pseudo_healthy_supervised


log = logging.getLogger("proposed_executor")

class pseudo_healthy_supervised_executor(Executor):
...
if __name__=='__main__':
    midl_model = pseudo_healthy_supervised(EasyDict(EXPERIMENT_PARAMS))
    midl_model.build()
    executor_midl_model = pseudo_healthy_supervised_executor(EasyDict(EXPERIMENT_PARAMS), midl_model)
