...
EXPERIMENT_PARAMS = {
    ...
    "folder": "experiments/pseudo_healthy_%s_%s_%d_"%(dataset, modality, split)+t.split(' ')[0]+'-'+t.split(' ')[1].replace('.','-').replace(':','-'),  # Folder to store experiment results.
    "epochs": 300,
    "batch_size": 4,
    ...
    "model": "pseudo_healthy_supervised.pseudo_healthy_supervised",  # Selected GAN architecture.
    "executor": "pseudo_healthy_supervised_executor.pseudo_healthy_supervised_executor",  # Selected experiment executor.
    "out_channels": 1,
    "gp_weight":10,
    "outputs": 1,
    "filters":32,
